...
Revision ID: 737fa4230314
Revises: 
Create Date: 2026-05-21 19:25:49.250118
...
def upgrade() -> None:
    ...
    op.alter_column('carts', 'id',
               ...
               autoincrement=True,
               existing_server_default=sa.text("nextval('carts_id_seq'::regclass)"))
...
def downgrade() -> None:
    ...
    op.create_table('bot_engine_buys',
    sa.Column('id', sa.BIGINT(), sa.Identity(always=False, start=1, increment=1, minvalue=1, maxvalue=9223372036854775807, cycle=False, cache=1), autoincrement=True, nullable=False),
    ...
    sa.Column('buy_datetime', postgresql.TIMESTAMP(timezone=True), autoincrement=False, nullable=False),
    sa.Column('is_refunded', sa.BOOLEAN(), autoincrement=False, nullable=False),
    sa.PrimaryKeyConstraint('id', name=op.f('bot_engine_buys_pkey')),
    sa.UniqueConstraint('buyer_id', name=op.f('bot_engine_buys_buyer_id_key'), postgresql_include=[], postgresql_nulls_not_distinct=False)
    )
